...
normalize_city <- function(x, country = "DE") {
  ...
  if (country == "DE") return(normalize_city_de(x))
  if (country == "CH") return(normalize_city_ch(x))
  if (country == "AT") return(normalize_city_at(x)) 
  if (country == "LI") return(normalize_city_de(x))
  
  if (country %in% c("FR", "BE", "LU")) return(normalize_city_fr_base(x, country))
  if (country %in% c("IT")) return(normalize_city_it(x))
  if (country %in% c("ES")) return(normalize_city_es(x))
  if (country %in% c("PT")) return(normalize_city_pt(x))
  
  if (country %in% c("NL")) return(normalize_city_nl(x))
  
  if (country %in% c("DK", "NO", "SE")) return(normalize_city_scandi(x, country))
  if (country %in% c("FI")) return(normalize_city_fi(x))
  if (country %in% c("IS")) return(normalize_city_is(x))
  
  if (country %in% c("PL")) return(normalize_city_pl(x))
  if (country %in% c("CZ", "SK")) return(normalize_city_cz_sk(x, country))
  if (country %in% c("HU")) return(normalize_city_hu(x))
  if (country %in% c("RO", "MD")) return(normalize_city_ro(x))
  
  if (country %in% c("HR", "SI", "BG", "MK")) return(normalize_city_slavic_south(x, country))
  if (country %in% c("EE", "LV", "LT")) return(normalize_city_baltic(x, country))
  if (country %in% c("EL", "CY")) return(normalize_city_greek(x))
